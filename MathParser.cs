...
namespace stringParse
{
    class MathParser
    {
        public static double Parser(string func, double x)
        {
            double result;
            if (func.Contains("(") || func.Contains(")"))
            {
                result = Brackets(func, x);
            }
            else result = Calculate(StringToArr(func, x), x);
            return result;
        }
        static double Brackets(string func, double x)
        // Если есть скобки
        {
            int open = 0, close = 0;
            string Temp, Temp2;
            ...
            do
            {
                ...
                Temp2 = Temp.Trim('(', ')');
                Temp3 = Calculate(StringToArr(Temp2, x), x);
                if (Temp3 >= 0)
                {
                    func = func.Replace(Temp, Temp3.ToString());
                }
                else func = func.Replace(("+" + Temp), Temp3.ToString());
            }
            while (func.Contains("("));
            return Calculate(StringToArr(func, x), x);
        }
        static string[] StringToArr(string func, double x)
        {
            if (func.Contains('('))
            {
                func = Brackets(func, x).ToString();
            }
        ...
        static double Calculate(string[] args, double x)
        {
            double result;
            double temp;
